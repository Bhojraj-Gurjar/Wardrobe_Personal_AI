...
export function enrichFaceTraits(storedTraits, fullFaceAnalysis) {
  if (!fullFaceAnalysis) {
    return storedTraits;
  }
...
export function enrichBodyTraits(storedTraits, fullBodyAnalysis, profile) {
  const merged = { ...storedTraits };
...
export function hasBasicAvatarTraits(context) {
  return Boolean(
    context.profile?.gender
    || context.profile?.age
    || context.profile?.body_type
    || context.profile?.skin_tone
    || context.faceTraits?.face_shape
    || context.faceTraits?.faceShape
    || context.faceTraits?.hair_color
    || context.faceTraits?.hairColor
    || context.bodyTraits?.body_type
    || context.bodyTraits?.bodyType
    || context.bodyTraits?.body_shape
    || context.bodyTraits?.bodyShape,
  );
}

export function hasPremiumAvatarTraits(context) {
  const face = context.faceTraits || {};
  const body = context.bodyTraits || {};
  const skinTone = face.skin_tone
    || face.skinTone
    || context.profile?.skin_tone;
  ...
  const hasBeardAnalysis = (
    face.beard_type !== undefined && face.beard_type !== null
  ) || (
    face.beardType !== undefined && face.beardType !== null
  );
...
/** Future 3D twin may require additional depth/body scan signals. */
export function hasDigitalTwin3DTraits(context) {
  return hasPremiumAvatarTraits(context);
}

export function buildTraitFingerprint(context) {
  const payload = {
    face: {
