...
const CATEGORY_STYLE_WEIGHTS = {
  casual: { Casual: 1, Streetwear: 0.45 },
  formal: { Formal: 1, Classic: 0.75 },
  luxury: { Classic: 0.65, 'Avant-garde': 0.55, Minimalist: 0.35 },
  sportswear: { Streetwear: 0.85, Casual: 0.55 },
  athleisure: { Casual: 0.75, Streetwear: 0.5 },
  minimalist: { Minimalist: 1 },
  vintage: { Classic: 0.7, 'Avant-garde': 0.45 },
  streetwear: { Streetwear: 1, Casual: 0.4 },
  business: { Formal: 0.8, Classic: 0.7, Minimalist: 0.35 },
  workwear: { Classic: 0.65, Formal: 0.55 },
  outerwear: { Classic: 0.45, Casual: 0.4 },
  accessories: { Classic: 0.35, Minimalist: 0.3 },
};

const PERSONALITY_STYLE_WEIGHTS = {
  'Minimal Professional': { Minimalist: 0.35, Classic: 0.25 },
  'Business Casual': { Classic: 0.3, Formal: 0.25, Casual: 0.2 },
  'Streetwear Enthusiast': { Streetwear: 0.4, Casual: 0.25 },
  'Luxury Executive': { Classic: 0.35, Formal: 0.3, Minimalist: 0.2 },
  'Athletic Lifestyle': { Casual: 0.3, Streetwear: 0.35 },
};
...
const BUDGET_RANGE_LABELS = {
  ECONOMY: { min: 20, max: 50, label: '$20 – $50' },
  MID_RANGE: { min: 50, max: 300, label: '$50 – $300' },
  PREMIUM: { min: 300, max: 800, label: '$300 – $800' },
  LUXURY: { min: 800, max: 2000, label: '$800+' },
};
...
function buildStyleScores(categoryAffinity, fashionPersonality) {
  const scores = Object.fromEntries(STYLE_AXES.map((axis) => [axis, 0]));
  ...
  return Object.fromEntries(
    STYLE_AXES.map((axis) => [axis, clamp((scores[axis] / maxScore) * 100)]),
  );
}

export function deriveStyleAttributes(categoryAffinity, fashionPersonality) {
  const scores = buildStyleScores(categoryAffinity, fashionPersonality);

  return Object.fromEntries(
    STYLE_ATTRIBUTE_KEYS.map((key) => [key, scores[key] ?? 0]),
  );
}

export function deriveStyleRadar(categoryAffinity, fashionPersonality) {
...
export function derivePercentileLabel(confidenceScore) {
...
export function deriveBudgetDisplay(budgetRange, averageSpending) {
  ...
  return {
    ...
    spendProgress:
      Number.isFinite(spend) && spend > 0
        ? clamp(((spend - range.min) / (range.max - range.min)) * 100)
        : 0,
  };
}
...
export function deriveHistoryTimeline(historyItems, currentScore, currentUpdatedAt) {
  const now = new Date();
  const months = [];
  ...
  for (const item of historyItems || []) {
    ...
    const score = Number(
      item.fashionConfidenceScore ?? item.fashion_confidence_score ?? 0,
    );
...
export function deriveWeeklyGrowth(historyItems, currentScore) {
  const now = Date.now();
  ...
  for (const item of historyItems || []) {
    ...
    if (baseline === null || archivedAt > baseline.archivedAt) {
      baseline = { archivedAt, score };
    }
  }

  if (!baseline) {
    const earliest = [...(historyItems || [])]
      .map((item) => ({
      ...
      .filter((item) => !Number.isNaN(item.archivedAt))
      .sort((left, right) => left.archivedAt - right.archivedAt)[0];

    baseline = earliest || { archivedAt: weekAgo, score: normalizedCurrent };
  }

  return clamp(normalizedCurrent - clamp(baseline.score));
}

export function formatBrandAffinityList(brandAffinity) {
