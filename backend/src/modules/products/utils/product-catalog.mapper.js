...
export function resolveLegacyCategoryId(dto, data = {}) {
...
export function resolveLegacyBrandId(dto, data = {}) {
...
export function resolveCatalogImages(dto) {
...
export function formatCatalogProduct(product) {
  ...
  return {
    ...formatted,
    avatarWearable: formatAvatarWearable(formatted),
    integration: formatProductIntegrationProfile(product),
  };
}

export function mapCreateOrUpdateProductData(dto, options = {}) {
  const { allowSku = true } = options;
  const data = {};

  if (allowSku && dto.sku !== undefined) data.sku = dto.sku;  if (dto.name !== undefined) data.name = dto.name;
