  ...
  async onModuleInit() {
    ...
    try {
      ...
      await this.ensureCollection(
        this.fashionDnaCollection,
        this.fashionDnaVectorSize,
      );
      await this.ensureCollection(
        this.faceAnalysisCollection,
        this.faceAnalysisVectorSize,
      );
      await this.ensureCollection(
        this.bodyAnalysisCollection,
        this.bodyAnalysisVectorSize,
      );
      await this.ensureCollection(
        this.digitalAvatarCollection,
        this.digitalAvatarVectorSize,
      );
      this.logger.log(
        `Qdrant collections ready (face: ${faceCollection}, products: ${this.collection}, fashion_dna: ${this.fashionDnaCollection}, face_analysis: ${this.faceAnalysisCollection}, body_analysis: ${this.bodyAnalysisCollection}, digital_avatar: ${this.digitalAvatarCollection})`,
      );
    } catch (error) {
  ...
  async ensureCollection(collectionName, vectorSize) {
    ...
    if (exists) {
      ...
      if (currentSize !== null && currentSize !== vectorSize) {
        this.logger.warn(
          `Qdrant collection "${collectionName}" has vector size ${currentSize}, expected ${vectorSize}. Recreating collection.`,
        );
    ...
    if (!exists) {
      this.logger.log(
        `Creating Qdrant collection "${collectionName}" (${vectorSize} dimensions)`,
      );
    ...
    try {
      ...
      return {
        ...
        face_collection: {
          ...
          actual_vector_size: await this.getCollectionVectorSize(
            config.faceCollection,
          ).catch(() => null),
        },
        ...config,
      };
    ...
    if (options.excludeUserId) {
      searchParams.filter = {
        must_not: [
          {
            key: 'user_id',
            match: { value: options.excludeUserId },
          },
        ],
      };
    }
