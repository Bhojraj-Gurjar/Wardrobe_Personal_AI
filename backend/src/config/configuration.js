...
export default () => ({
  ...
  face: {
    ...
    similarityThreshold:
      parseFloat(process.env.FACE_SIMILARITY_THRESHOLD) || 0.40,
    registrationDuplicateThreshold:
      parseFloat(process.env.FACE_REGISTRATION_DUPLICATE_THRESHOLD) || 0.45,
    similarityUncertain:
  ...
  faceAnalysis: {
    collection:
      process.env.QDRANT_FACE_ANALYSIS_COLLECTION || 'face_analysis_vectors',
    vectorSize:
      parseInt(process.env.FACE_ANALYSIS_VECTOR_SIZE, 10)
      || parseInt(process.env.DNA_VECTOR_SIZE, 10)
      || 384,
  },
  bodyAnalysis: {
    collection:
      process.env.QDRANT_BODY_ANALYSIS_COLLECTION || 'body_analysis_vectors',
    vectorSize:
      parseInt(process.env.BODY_ANALYSIS_VECTOR_SIZE, 10)
      || parseInt(process.env.DNA_VECTOR_SIZE, 10)
      || 384,
  },
  digitalAvatar: {
    collection:
      process.env.QDRANT_DIGITAL_AVATAR_COLLECTION || 'digital_avatar_vectors',
    vectorSize:
      parseInt(process.env.DIGITAL_AVATAR_VECTOR_SIZE, 10)
      || parseInt(process.env.DNA_VECTOR_SIZE, 10)
      || 384,
  },
  aiService: {
