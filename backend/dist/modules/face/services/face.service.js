...
let FaceService = class FaceService {
    ...
    async getFacePhoto(userId) {
        ...
        if (registration.face_image_url) {
            const exists = await this.faceImageStorageService.faceImageExists(registration.face_image_url);
            if (!exists) {
                this.logger.warn(`Stale face image reference for user ${userId}: ${registration.face_image_url}`);
