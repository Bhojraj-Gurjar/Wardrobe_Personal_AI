...
export function ColorAffinityCard({
  ...
  return (
    <section className={fashionDnaCardShell(className)}>
      <h3 className={fashionDnaCardTitleClass}>Color Affinity</h3>
      ...
      {colors.length ? (
        <>
          <div className="mt-3 grid grid-cols-5 gap-2 md:mt-5 md:gap-3">
            {colors.map((color) => (
              <div key={color.key} className="flex flex-col items-center gap-2 text-center">
                <span
                  className="size-9 rounded-full border border-white/10 shadow-inner md:size-12"
