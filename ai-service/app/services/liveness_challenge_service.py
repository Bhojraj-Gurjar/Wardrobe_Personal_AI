...
class LivenessChallengeService:
    ...
    def analyze_frames(
        ...
        *,
        ...
        min_detection = self._settings.face_hold_still_min_detection_score
        ...
        self._validate_anti_spoof(rgb_frames, detections, metrics_list)
    ...
    def _validate_face_geometry(self, rgb: np.ndarray, detection: FaceDetection) -> None:
        ...
        if face_width / max(face_height, 1.0) < 0.45 or face_width / max(face_height, 1.0) > 2.2:
    ...
    def _validate_hold_still(self, metrics_list: list[FrameMetrics]) -> None:
        settings = self._settings
        min_ear = settings.face_blink_ear_threshold * 0.75

        for metric in metrics_list:
            if metric.detection_score < settings.face_hold_still_min_detection_score:
                raise FaceValidationError(
                    "We couldn't verify your face clearly. Please try again.",
                    "liveness_failed",
                )
            if metric.ear < min_ear:
                raise FaceValidationError(
                    "We couldn't verify your face clearly. Please try again.",
                    "liveness_failed",
                )
